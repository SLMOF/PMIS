...
export const TableConfigs = <T extends BaseModel>({ callback, filters }: Props<T>) => {
    ...
    const showFiltersOnClickHandler = () => {
        MySwal.fire({
            ...
            width: 900,
            html: <ShowFilters callback={applyFilter} fields={filters! as { key: string, format: string, data?: { id: number, name: string }[] }[]} />
        })
    }
...
interface miniProps {
    ...
    fields: { key: string, format: string, data?: { id: number, name: string }[] }[];
}

export const ShowFilters: React.FC<miniProps> = ({ callback, fields }) => {

    const [inputType, setInputType] = useState<FilterableType>('text');
    const [selectBoxData, setSelectBoxData] = useState<{ id: number, name: string }[] | undefined>();
    ...
    const onSubmit = (data: any, e: any) => {
        ...
        if (data.filterField && data.filterOperation && data.filterValue) {
            filters.push({ type: 'filter', field: data.filterField, operation: data.filterOperation, value: data.filterValue });
        }
        ...
        data.beginningPage && filters.push({ type: 'page', value: data.beginningPage });
                ...
                <div className="row">
                    ...
                    <div className="col-9">
                        <div className="row">
                            ...
                            <div className="col">
                                <select className="custom-select" name="filterOperation" ref={register()}>
                                    <option value=""> </option>
                                    {
